...
def gather_all(spin,N,gsw,basename):
  ''' 
  ...
  for j in range(1,N+1):
    ...
    f=basename+'/S'+str(spin)+'_gsw'+str(np.round(gsw,2))+'_'+str(j)+'.vmc.gosling.json' 
    print(f)
...
if __name__=='__main__':
  ...
  for gsw in np.arange(0.1,1.0,0.1): 
    gather_all(spin,N,gsw,basename='spin'+str(spin)+'gsw'+str(np.around(gsw,2)))
