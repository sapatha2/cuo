...
def gather_all(detgen,N,Ndet,gsw,basename):
  ...
  for j in range(N+1):
    ...
    for name in ['gs0','gs1','gs2','gs3','gs4','gs5']:
      ...
      #dat=np.array([energy,energy_err]+list(one_body)+list(U)+list(V)+list(J))
      #d=pd.DataFrame(dat[:,np.newaxis].T,columns=['energy','energy_err']+one_labels+U_labels+V_labels+J_labels)
