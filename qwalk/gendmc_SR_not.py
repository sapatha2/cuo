...
template=[]
with open("dmc_template_SR_not","r") as f:
  for line in f:
...
for method in ['B3LYP']:
  for basis in ['vtz']:
    for i in range(1,N+1):
      ...
      fname=basename+".dmc_E_SR_"+str(i)+cutoff+"_not"
      fout=open(fname,"w")
...
for method in ['B3LYP']:
  for basis in ['vtz']:
    for i in range(1,N+1):
      basename=el+basis+str(charge)+"_"+method+".dmc_E_SR_"+str(i)+cutoff+"_not"
      ...
      "cp "+cpypath+".dmc_E_SR_"+str(i)+cutoff+"_not .\n"+\
